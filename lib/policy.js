...
function sanitize(value, token, depth = 0) {
  ...
  if (value && typeof value === "object") return Object.fromEntries(Object.entries(value).map(([k, v]) => [k,
    /^(accessToken|refreshToken|apiKey|clientSecret|authorization|password|secret|privateKey|token|protectionBypass)$/i.test(k.replace(/[_-]/g, "")) || (k === "value" && ["encrypted", "sensitive"].includes(value.type))
