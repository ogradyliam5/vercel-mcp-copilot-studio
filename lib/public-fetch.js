...
function parsePublicUrl(input, approved) {
  ...
  // No query strings: a denylist of credential names cannot cover arbitrary
  // API keys, signatures or application-specific auth parameters.
  if (url.search) throw new Error("Query strings are not accepted for public content URLs");
  return url;
}
