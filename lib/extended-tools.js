...
const registrarTeamId = { ...str("Actual team_ ID for registrar APIs; slugs are not accepted. Omit for token-inferred context."), pattern: "^team_[A-Za-z0-9]+$" };
...
function checkFiles(files) {
  ...
  for (const f of files) {
    ...
    seen.add(f.file);
    if (f.encoding === "base64" && (f.data.length % 4 || !/^[A-Za-z0-9+/]*={0,2}$/.test(f.data))) throw new Error("Invalid base64 source file");
    ...
    if (f.encoding === "base64" && bytes.toString("base64") !== f.data) throw new Error("Non-canonical base64 source file");
    total += bytes.length;
    // Normalize both caller encodings to the wire format used by Vercel CLI.
    // Validate decoded bytes before encoding, and do not mutate caller data.
    prepared.push({ file: f.file, data: bytes.toString("base64"), encoding: "base64" });
  }
...
add("deploy_to_vercel", "Deploy inline source files to Vercel. Starts a build and can create a project. Requires operator-enabled writes. Returns pending state, not verified success.", {
  ...
  const files = checkFiles(a.files);
  return deployed(await vercelFetch(token, "POST", "/v13/deployments", scoped(a), {
    name: a.name, files, ...fields(a, ["projectSettings"]), ...(a.target === "production" ? { target: "production" } : {}),
  }));
}, { write: true });
...
add("check_domain_availability_and_price", "Check domain availability and quoted registration/renewal prices. Does not purchase or reserve anything. Up to 5 domains per call.", {
  names: { type: "array", minItems: 1, maxItems: 5, uniqueItems: true, items: { ...str("ASCII/punycode domain", 253), pattern: "^(?:[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?\\.)+[a-zA-Z]{2,63}$" } }, teamId: registrarTeamId,
...
add("get_domain_order", "Read domain registration order status. A purchasing order is not yet completed. Does not buy anything.", {
  orderId: str("Domain order ID"), teamId: registrarTeamId,
...
}, ["teamId", "projectId", "runId"], async (token, a) => trimTrace(sanitize(await dashboardFetch(token, "/api/observability/agent-runs", { ...runQuery(a), runId: a.runId, trace: 1 }), token), a.maxFieldLength ?? 8000), { cliBacked: true });
