...
| `deploy_to_vercel` | Implemented; see limits | Inline source deployment. Requires name, files and explicit preview/production target. Maximum 100 files and 1048576 decoded bytes total. No local filesystem access. Production requires a separate operator flag. Build state is returned; success is not assumed. All inline files are normalized to base64 on the API wire after decoded-byte checks. |
...
| `check_domain_availability_and_price` | Implemented; see limits | Read-only availability and exact price responses for 1–5 domains. Optional years 1–10; omitted uses the TLD minimum. Does not quote a binding purchase or reserve a domain. Registrar team scoping requires an actual team_ ID; slugs are rejected because these contracts do not expose a slug parameter. |
...
| `buy_pro` | **Not implemented** | Vercel CLI uses POST /v1/billing/buy with item.type=subscription and planSlug=pro. This identifies an initiation endpoint, not the official signed quote/confirm, replay and recurring-billing approval guarantees. Those still require a verified design and explicit billing approval before exposure. |
...
| `get_domain_order` | Implemented; see limits | Read-only order status. purchasing is not completed. No purchase is performed. Registrar team scoping requires an actual team_ ID; slugs are rejected because these contracts do not expose a slug parameter. |
...
## 2026-09-16 review revalidation

The refreshed OpenAPI snapshot has SHA-256 3fe84ea9ee221354b9c2873936f66e262264e0d7baf5151d2b0fc9b63148d0da. Its inline-file schema explicitly permits base64 and utf-8. The review assertion that the API permits only base64 was not established; this bridge nevertheless now normalizes to base64 to use the common format supported by the API and Vercel CLI. Unicode, empty and binary payloads have byte-preservation tests.

Further public CLI evidence identifies Pro subscription purchase initiation at /v1/billing/buy, but does not establish the official signed quote/confirm or durable replay guarantees. Source: https://github.com/vercel/vercel/blob/c628be7835e03a965b93e9cf9e2bd5ac2acbf5eb/packages/cli/src/util/buy/create-purchase.ts and types.ts in the same folder. No billing mutation was performed.
